<x-partials.form-section submit="updateProfileInformation">
    ...
    <x-slot name="form">
        ...
        <x-select id="nationality" name="nationality" label="nationality *" group-class="col-span-3" >
            @php ($nationality = ['Indian', 'NRI'])
            @foreach ($nationality as $nationality)
                <option value="{{$nationality}}"  >{{$nationality}}</option>
            @endforeach
        </x-select>
        <x-select id="state" name="state" label="state *" group-class="col-span-3" >
            @php ($state = ['Andaman and Nicobar Islands',
    'Andhra Pradesh',
    'Arunachal Pradesh',
    'Assam',
    'Bihar',
